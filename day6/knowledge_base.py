...
class KnowledgeBase:
    ...
    def rebuild(self, notes_dir: str = NOTES_DIR, batch_size: int = 8):
        ...
        for md_file in md_files:
            raw = Path(md_file).read_text(encoding="utf-8")
            # 剥掉 frontmatter —— 它是元数据，不该被切分/检索
            meta, text = frontmatter.parse(raw)
            ...
            for idx, chunk in enumerate(chunks):
                ...
                md = {"file": filename, "chunk_idx": idx}
                # 把 tags 也塞进 metadata，方便未来按 tag 过滤
                if meta.get("tags"):
                    md["tags"] = ",".join(meta["tags"])
    ...
    def read_full_note(self, filename: str, notes_dir: str = NOTES_DIR, max_chars: int = 8000) -> str:
        """
        读整篇笔记原文（剥掉 frontmatter，避免 LLM 把 tags/summary 当正文）。
    ...
    def read_meta(self, filename: str, notes_dir: str = NOTES_DIR) -> dict:
        """读一个笔记的 frontmatter（不存在的键返回空 dict）"""
        path = os.path.join(notes_dir, filename)
        if not os.path.exists(path):
            raise FileNotFoundError(f"笔记不存在：{filename}")
        raw = Path(path).read_text(encoding="utf-8")
    ...
    def update_meta(self, filename: str, updates: dict, notes_dir: str = NOTES_DIR) -> dict:
        """
        更新一个笔记的 frontmatter（合并覆盖），把新的写回文件。
        list 类型的字段做去重合并（tags: [a, b] + [b, c] → [a, b, c]）。
        返回合并后的完整 meta。
        """
        path = os.path.join(notes_dir, filename)
        if not os.path.exists(path):
        ...
        for k, v in updates.items():
            if isinstance(v, list) and isinstance(meta.get(k), list):
                seen = set(meta[k])
                merged = list(meta[k])
                for item in v:
                    if item not in seen:
        ...
        Path(path).write_text(frontmatter.dump(meta, body), encoding="utf-8")
        return meta
