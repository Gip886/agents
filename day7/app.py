...
Day 7 相比 Day 6：
  - 工具卡片头显示 💭 Thought
  - 侧栏加了 web_search 状态（TAVILY_API_KEY 是否配置）
...
with st.sidebar:
    ...
    if os.getenv("TAVILY_API_KEY"):
        ...
        st.caption(
            "去 [tavily.com](https://tavily.com) 免费申请 (1000 次/月)，"
            "写到 `.env` 里 `TAVILY_API_KEY=tvly-xxx`"
        )

    st.divider()
    st.caption("Day 7 · [GitHub](https://github.com/Gip886/agents)")
...
st.title("🤖 知识助手 v4")
st.caption("基于你私人笔记的问答助手 · RAG + Tool Use + 记忆管理 + ReAct + 联网兜底")
...
def render_turn_trace(turn: TurnResult):
    ...
    for r in turn.rounds:
        ...
        with st.status(
            ...
            # 💭 Thought：Day 7 新增，把这轮 LLM 在 content 里写的"我要做什么"展示出来
            if r.thought:
