"""
知识助手 Agent v4（Day 7：Thought + web_search）
====================================
相比 Day 6 的关键改动：
  1. Thought：让 LLM 在每次调用工具前先在 content 里写一句"打算做什么"，
     RoundTrace 存下来，UI/CLI 展示它。就是最朴素的 ReAct（Reason→Act→Observe）。
  2. web_search：Tavily 联网兜底工具（本地笔记没答案时才用）。

跑法：
  python agent.py       # CLI 模式
  streamlit run app.py  # Web UI 模式
...
5. **一次可以并行调用多个工具**以提高效率（比如 search_notes + web_search 同时来）。
...
class RoundTrace:
    """一轮 LLM 调用的完整记录"""
    round_num: int
    thought: str = ""       # 🆕 Day 7：LLM 在调工具前写的"我要做什么"
...
class KnowledgeAgent:
    ...
    def run_turn(self, user_input: str) -> TurnResult:
        ...
        for round_num in range(1, self.max_rounds + 1):
            ...
            # 有 tool_calls：这轮的 content 就是"思考"（Day 7 新增）
            # OpenAI 允许 tool_calls 消息同时带 content。system prompt 已教育 LLM 写它。
...
def _print_turn(result: TurnResult):
    """把 TurnResult 打印成 Day 7 版思考轨迹（含 Thought）"""
...
def main():
    ...
    print("\n" + "=" * 60)
    print("🤖 知识助手 v4 已启动（Day 7：ReAct + web_search；也可用 streamlit run app.py）")
